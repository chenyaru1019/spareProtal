...
$.MvcSheet.Loaded = function () {
    ...
    // 自定义文件格式 支付凭证
    $("#Attachment2").xnfile().xnfile("option", "multiple", true).xnfile("option", "basePath", "\\cta\\returnFileBatch");
    ...
    var dtl_back = $.MvcSheetUI.GetElement("GetBackBatchFKTbl").SheetGridView();
        ...
        $.ajax({
            ...
            success: function (ret) {    //这是处理后执行的函数，msg是处理页返回的数据
                ...
                if (ret.SUCCESS == true) {
                    ...
                    for (var i = 0; i < ret.InstanceActivity.length; i++) {
                        ...
                        if (CurActivity == "End") {
                            ...
                            if (ret.InstanceActivity[i].ActivityName == "财务确认") {
                                options += '<option value="' + ret.InstanceActivity[i].ActivityCode + '">' + ret.InstanceActivity[i].ActivityName + '</option>';
                            }
                        }
                ...
                confirmButton.click(function () {
                    ...
                    GotoBackPage("BatchFK", CurActivity);
...
function viewBack(el) {
    var rowIndex = el.parentElement.parentElement.rowIndex;
    var WorkItemId = $.MvcSheetUI.GetControlValue('GetBackBatchFKTbl.WorkItemId', rowIndex - 1);
