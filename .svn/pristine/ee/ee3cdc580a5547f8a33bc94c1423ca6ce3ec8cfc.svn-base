...
function addBtn(value, row) {
    //        console.log(row.gys);
    var str = "<a id='TableOperate' class='btn btn-primary btn-xs' data-toggle='modal'>处理</a>";

    return [str].join("")
}
var SInstanceName = "";
$('#table_UnfinishedTask').bootstrapTable({
    //height: 580,
    //        showRefresh: true,
    toolbar: '#toolbar',
    //search : true,
    ...
    striped: true,
    method: 'get',
    url: "ContractHandler.ashx?Command=getUnfinishedTask",
...
function Search() {
    ...
    $('#table_UnfinishedTask').bootstrapTable('refresh');
}
// 查询
//function Search() {
//    var CurrentUser = eval("(" + window.localStorage.getItem('getCurrentUser') + ")");
