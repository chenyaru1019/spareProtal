    ...
    .controller('AppCtrl', ['$rootScope', '$scope', '$translate', '$localStorage', '$window', '$interval',
        ...
        function ($rootScope, $scope, $translate, $localStorage, $window, $interval, $http, $state, $stateParams, $location, $document, $modal, ControllerConfig, $filter, $compile, $timeout, menuhandle) {
            ...
            $scope.li_click = function (i) {
                $scope.focus = i;
                window.localStorage.setItem("li_num", JSON.stringify($scope.focus));
            };
            //刷新
            $scope.refresh = function () {
                // console.log(window.localStorage.getItem('li_num'));
                $scope.focus = window.localStorage.getItem('li_num');
                // 获取当前用户
                $http({
            ...
            //获取待办流程数量
            $scope.GetUnItemCount = function () {
                //var user =localStorage.getItem('getCurrentUser');
                var CurrentUser = this.user;//JSON.parse(user);
                // 
                $.ajax({
                    type: "POST",    //页面请求的类型
                    url: "/Portal/Sheets/Contract/ContractHandler.ashx?Command=getUnfinishedTaskCount",   //处理页的相对地址
                    data: {
                        CurrentUser: CurrentUser.ObjectID,
                    },
                    async: false,
                    success: function (result) {    //这是处理后执行的函数，msg是处理页返回的数据
                        $scope.UnfinishedCount = result.UnfinishedCount;
                        $scope.FinishedCount = result.FinishedCount;
                    }
                });
            ...
            $scope.registerAutoRefresh = function () {
                $scope.autoRefresh = $interval(function () {
                    $scope.GetItemCount();
                    $scope.GetUnItemCount();
                }, 60 * 1000);
            }

            // 退出系统
            $scope.loginOut = function () {
                window.localStorage.setItem("li_num", 0);
                $http({
            ...
            $scope.$on("LoginIn", function (event, args) {
                if (args.Success) {
                    ...
                    if ($('#ck_rmbUser').is(':checked')) {
                        window.localStorage.setItem("getCurrentUserName", JSON.stringify($scope.user));
                    }else {
                        window.localStorage.setItem("getCurrentUserName", null);
                    }
                    $scope.focus = 0;
                    window.localStorage.setItem("getCurrentUser", JSON.stringify($scope.user));
                    // 隐藏当前窗体
            ...
            //打开常用意见
            $scope.showUserMyCommets = function () {
                layer.open({
                    type: 2,
                    title: '常用意见',
                    shadeClose: true,
                    shade: 0.8,
                    area: ['760px', '80%'],
                    content: '#/app/Workflow/MyComments' //iframe的url
                });
            };
