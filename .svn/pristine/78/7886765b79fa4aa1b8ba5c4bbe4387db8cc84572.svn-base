...
app.controller('MyUnReadWorkItemController', ['$scope', '$rootScope', '$translate', '$http', '$state', '$compile', '$interval', 'ControllerConfig', 'jq.datables',
    function ($scope, $rootScope, $translate, $http, $state, $compile, $interval, ControllerConfig, jqdatables) {
        ...
        $scope.getColumns = function () {
            ...
            columns.push({ "mData": "BusinessSubType", "sClass": "center " });
            columns.push({ "mData": "MsgTitle", "sClass" : "center" });
                        ...
                        ////查询添加 编号、简称
                            for (var i = 0; i < json.Rows.length; i++) {
                                $.ajax({
                                    ...
                                    async: false,
                                    success: function (e) {
                                        json.Rows[i]["BusinessNumber"] = e.OperationNumber;
                                        //json.Rows[i]["ProjectAbbreviation"] = e.ProjectAbbreviation;
                                        json.Rows[i]["BusinessType"] = e.OperationType;
                                        json.Rows[i]["BusinessSubType"] = json.Rows[i].InstanceName.split(".")[0].replace("流程", "");
                                        if (json.Rows[i].InstanceName.split(".")[0] != '') {
                                            json.Rows[i]["MsgTitle"] = "【" + json.Rows[i].InstanceName.split(".")[0] + "】" + "已完成";
                                        } else {
                                            json.Rows[i]["MsgTitle"] = "";
                                        }
                                    }
                                });
                            }
