(function($) {
    $.widget("nmk.xnfile", {
        ...
        _onFileChanged: function() {
            var files = $("input[type=file]", this.element).prop("files");
            if (files.length > 0) {
                var options = this._getOptions(this.element);
                for (var i = 0; i < files.length; i++) {
                    options.pendingFiles.push(files[i]);
                }
        ...
        _checkState: function() {
            var _this = this;
            var options = this._getOptions(this.element);
            if (options.pendingFiles.length > 0) {
                for (var i = 0; i < options.pendingFiles.length; i++) {
                    var filename = options.pendingFiles[0].name.split("\\").pop();
                    $.getJSON("/Portal/Sheets/xnfile/XnfileHandler.ashx",
                        ...
                        function(data) {
                            if (data.found === "true") {
                                if (options.multiple) {
                                    if (!options.files.includes(data.filename)) {
                                        options.files.push(data.filename);
                                    }
                                }
                                else {
                                    $("div.file2-done>div.data", _this.element).remove(); 
                                    options.files = [data.filename];
                                }
                                for (var j = 0; j < options.pendingFiles.length; j++) {
                                    if (data.filename === options.pendingFiles[j].name) {
                                        options.pendingFiles.splice(j, 1);
                                        break;
                                    }
                                }
                            }
        ...
        _setOption: function(key, value) {
            ...
            if (key === "multiple") {
                ...
                $("input[type=file]", this.element).prop("multiple", value);
            }
            else if (key == "basePath") {
                options.basePath = value;
            }
            else if (key == "readonly") {
                $("input[type=button]", this.element).prop("disabled", true);
            }
