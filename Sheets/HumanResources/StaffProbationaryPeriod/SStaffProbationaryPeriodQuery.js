...
$('#table_StaffProbationaryPeriodQuery').bootstrapTable({
    height: 510,
    //        showRefresh: true,
    toolbar: '#toolbar',
    //模拟数据
    columns: [{
        ...
        align: 'center',
        valign: 'middle',
        field: 'TalkingAboutGraphsAndTrends',
        width: '30%',
        title: '工作小结'
    }, {
        align: 'center',
...
$.MvcSheet.Loaded = function () {
    ...
    // 隐藏导航栏
    $("#main-navbar").hide();
    StaffProbationaryPeriodQuery();
}

// 查询
function StaffProbationaryPeriodQuery() {
    ...
    $("#table_StaffProbationaryPeriodQuery").bootstrapTable('removeAll');

    $.ajax({
        type: "POST",    //页面请求的类型
        url: "StaffProbationaryPeriodHandler.ashx?Command=getStaffProbationaryPeriodQuery",   //处理页的相对地址EmployeeLeaveHandler.ashx?Command=
        data: {
            StaffName: StaffName,
        ...
        success: function (ret) {    //这是处理后执行的函数，msg是处理页返回的数据
            for (var i = 0; i < ret.length; i++) {
                ...
                obj.TalkingAboutGraphsAndTrends = ret[i].TalkingAboutGraphsAndTrends;
                obj.WorkItemId = ret[i].WorkItemId;
                // 往bootstrapTable添加数据
                $("#table_StaffProbationaryPeriodQuery").bootstrapTable('insertRow', { index: i, row: obj });
            }
        }
    });
