...
$.MvcSheet.ActionDone = function (data) {
    if ($.MvcSheetUI.SheetInfo.ActivityCode == "cre_agreement") {
        ...
        if (agreement_sign == "true") {
            ...
            window.location.href = "/Portal/Sheets/AgreeMent/Agreement_sign/Agreement_sign.aspx?Mode=Originate&WorkflowCode=Agreement_sign&WorkflowVersion=" + Version_Agreement_sign + "&AgreeMent_number=" + AgreeMent_number;
        }
    } 
...
$.MvcSheet.Loaded = function () {
    ...
    var AgreeMent_number = $.MvcSheetUI.GetControlValue("AgreeMent_number");
    $(".AgreeMent_number_show").text(AgreeMent_number);
    ...
    // 自定义文件格式 写法1 其他文件 []
    $("#OtherFile").xnfile().xnfile("option", "multiple", true).xnfile("option", "basePath", "\\ptc\\protocal");
    ...
    $("#FileSignVersion").xnfile().xnfile("option", "multiple", true).xnfile("option", "basePath", "\\ptc\\" + AgreeMent_number + "\\归档");
    ...
    $("#BGFileSignVersion").xnfile().xnfile("option", "multiple", true).xnfile("option", "basePath", "\\ptc\\" + AgreeMent_number + "\\归档");
    ...
    if ($.MvcSheetUI.SheetInfo.ActivityCode == "agreement_excu") {
        ...
        if (hasPercent) {
            ...
            var len = dtl.RowCount;
            if (len > 0) {
                var AgencyFeeRemainRMB = $.MvcSheetUI.GetControlValue("YSList_Percent.AgencyFeeRemainRMB", 1);
                var AgencyFeeRemainWB = $.MvcSheetUI.GetControlValue("YSList_Percent.AgencyFeeRemainWB", 1);
