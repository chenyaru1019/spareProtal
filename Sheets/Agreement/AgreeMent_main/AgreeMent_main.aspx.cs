...
namespace OThinker.H3.Portal.Sheets.Agreement
{
    public partial class AgreeMent_main : OThinker.H3.Controllers.MvcPage
    {
        ...
        public override MvcViewContext LoadDataFields()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                AgreeMent_number = dt.Rows[0]["AgreeMent_number"].ToString();
                ReceiveAgencyFeeHidden = dt.Rows[0]["ReceiveAgencyFeeHidden"].ToString();
            }
            ...
            //协议审签的版本号
            int Version_Agreement_sign = OThinker.H3.Controllers.AppUtility.Engine.WorkflowManager.GetWorkflowDefaultVersion("Agreement_sign");
            this.ActionContext.InstanceData["Version_Agreement_sign"].Value = Version_Agreement_sign;
            ...
            System.Data.DataTable dt_gdfn = OThinker.H3.Controllers.AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteDataTable(
                    " select af.Agency_change,af.Agency_original from I_Agreement_file af  " +
            ...
            if (dt_gdfn.Rows.Count > 0)
            {
                ...
                {
                    if (dr["Agency_original"].ToString() != null && !("").Equals(dr["Agency_original"].ToString()))
                    {
                        if (!"".Equals(FileSignVer))
                        {
                            FileSignVer += ",";
                        }
                        FileSignVer += dr["Agency_original"].ToString();
                    }
                    if (dr["Agency_change"].ToString() != null && !("").Equals(dr["Agency_change"].ToString()))
                    {
                        if (!"".Equals(BGFileSignVer))
                        {
                            BGFileSignVer += ",";
                        }
                        BGFileSignVer += dr["Agency_change"].ToString();
                    }
                    i++;
                }
            ...
            if (dt_gd.Rows.Count > 0)
            {
                ...
                {
                    BizObjectSchema childSchema = this.ActionContext.Schema.GetProperty("AgreementFileRecordTbl").ChildSchema;
                    // 第一行
                ...
                this.ActionContext.InstanceData["AgreementFileRecordTbl"].Value = bizObjects;
            }
            ...
            System.Data.DataTable dt_bg = OThinker.H3.Controllers.AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteDataTable(
                   " SELECT ac.PayConditionOld, ac.PayConditionNew, " +
                    ...
                    "  where ac.AgreeMent_number = '" + AgreeMent_number + "' and ac.ObjectID = ins.BizObjectId  " +
            ...
            if (ls.Count > 0)
            {
                ...
                {
                    BizObjectSchema childSchema = this.ActionContext.Schema.GetProperty("Agreement_bg").ChildSchema;
                    // 第一行
                    bizObjects[i] = new BizObject(this.ActionContext.Engine, childSchema, this.ActionContext.User.UserID);
                ...
                this.ActionContext.InstanceData["Agreement_bg"].Value = bizObjects;
            }
            ...
            if (ls_All.Count > 0)
            {
                ...
                {
                    BizObjectSchema childSchema = this.ActionContext.Schema.GetProperty("Project_contracts").ChildSchema;
                    // 第一行
                    bizObjects[i] = new BizObject(this.ActionContext.Engine, childSchema, this.ActionContext.User.UserID);
                ...
                this.ActionContext.InstanceData["Project_contracts"].Value = bizObjects;
            }
