...
$.MvcSheet.Loaded = function () {
    ...
    // 自定义文件格式 合同变更书签字版
    $("#ContractChangeFileSignVer").xnfile().xnfile("option", "multiple", true).xnfile("option", "basePath", "\\cta\\" + ContractNo + "\\归档");
    var ContractChangeFileSignVer = $.MvcSheetUI.GetControlValue("ContractChangeFileSignVer");
    if (ContractChangeFileSignVer != "") {
        var arr = ContractChangeFileSignVer.split(",");
        $("#ContractChangeFileSignVer").xnfile("value", arr);
    }
    var IsHYFlg = $.MvcSheetUI.GetControlValue("IsHYFlg");
    if (IsHYFlg == "true") {
        // 自定义文件格式 协议变更书签字版
        $("#AgencyChangeFileSignVer").xnfile().xnfile("option", "multiple", true).xnfile("option", "basePath", "\\cta\\" + ContractNo + "\\归档");
        var AgencyChangeFileSignVer = $.MvcSheetUI.GetControlValue("AgencyChangeFileSignVer");
        if (AgencyChangeFileSignVer != "") {
            var arr = AgencyChangeFileSignVer.split(",");
            $("#AgencyChangeFileSignVer").xnfile("value", arr);
        }
        $(".HYDiv").show();
    } else {
        $(".HYDiv").hide();
    ...
    if ($.MvcSheetUI.SheetInfo.ActivityCode == "ActivityOrig") {
        setFileWriteable("ContractChangeFileSignVer");
        setFileWriteable("AgencyChangeFileSignVer");
    } else {
        setFileReadOnly("ContractChangeFileSignVer");
        setFileReadOnly("AgencyChangeFileSignVer");
    }

    // 回退节点是否显示，如果回退记录有，则显示，没有，则不显示
    var dtl_back = $.MvcSheetUI.GetElement("GetBackGDChangeTbl").SheetGridView();
...
$.MvcSheet.Validate = function () {
    ...
    // 填写申请单环节，设置  必填
    if ($.MvcSheetUI.SheetInfo.ActivityCode == "ActivityOrig") {
        var ContractChangeFileSignVer = $("#ContractChangeFileSignVer").xnfile("value");
        $.MvcSheetUI.SetControlValue("ContractChangeFileSignVer", ContractChangeFileSignVer);
        var IsHYFlg = $.MvcSheetUI.GetControlValue("IsHYFlg");
        if (IsHYFlg == "true") {
            var AgencyChangeFileSignVer = $("#AgencyChangeFileSignVer").xnfile("value");
            $.MvcSheetUI.SetControlValue("AgencyChangeFileSignVer", AgencyChangeFileSignVer);
        }
    }
    return true;
}
        ...
        $.ajax({
            ...
            success: function (ret) {    //这是处理后执行的函数，msg是处理页返回的数据
                ...
                confirmButton.click(function () {
                    ...
                    GotoBackPage("GDChange", CurActivity);
...
function viewBack(el) {
    ...
    var WorkItemId = $.MvcSheetUI.GetControlValue('GetBackGDChangeTbl.WorkItemId', rowIndex - 1);
