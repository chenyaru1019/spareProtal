...
jQuery.extend(
    $.MvcSheet,
    {
            ...
            if ($.MvcSheetUI.SheetInfo.IsMobile) {
                ...
                if (_Actions.length == 0) {
                    ...
                    _Actions.forEach(function (e) {
                        if (e.actionKey == "Reject" || e.actionKey == "Submit") {
                            ...
                            if (e.actionKey == "Reject") {
                                ...
                                e.text = "同意";
                                manager.ControlManagers[e.actionKey].Text = "同意"
                            }
                            var button = $("<button class=\"button foot-button " + strClass + " button-clear \"></button>")
                            button.text(e.text);
                            button.unbind("click." + e.actionKey).bind("click." + e.actionKey, function () { manager.ControlManagers[e.actionKey].ActionClick(); })
                    ...
                    if (moreActions.length > 0) {
                        var btnMore = $("<button class=\"button foot-button icon-left button-clear\">更多</button>")
                        var buttonsA = [];
        ...
        Submit: function (actionControl, text, destActivity, postValue) {
            ...
            layer.confirm(SheetLanguages.Current.ConfirmDo + "[" + text + "]" + SheetLanguages.Current.Operation + "?", function (index) {
                $.LoadingMask.Show(SheetLanguages.Current.Sumiting);
                var SheetPostValue = that.GetMvcPostValue(that.Action_Submit, destActivity, postValue);
                that.PostSheet(
                    {
        ...
        //提交2(提交时不需要弹出确认框)
        Submit2: function (actionControl, destActivity, postValue) {
            if ($.MvcSheetUI.SheetInfo.IsMobile) {
                var controls = $("#divSheet input[data-type='SheetTextBox']");
                controls.each(function () {
                    $(this).trigger("change");
        ...
        ControlRendered: function () {
            // 每个审批意见默认添加空格，避免查看审批意见显示不了审批人和时间等问题
            var SheetComment = $("div[data-type='SheetComment'] textarea").val();
            if (SheetComment==null || SheetComment==""){
                $("div[data-type='SheetComment'] textarea").val(' ');
            }
        },
        //驳回
            ...
            if (data.Successful) {
                ...
                if ($.MvcSheetUI.SheetInfo.IsMobile) {
                    ...
                    if (data.ClosePage) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "/Portal/Sheets/Contract/ContractHandler.ashx?Command=getWorkItemIDAndInsNameByOID",   //处理页的相对地址
                            data: {
                            ...
                            success: function (data2) {
                                if (data2 != null) {
                                    ...
                                    // 跳转到下一个节点
                                    if (data2.InstanceName != null && data2.InstanceName == '合同主流程') {
                                            ...
                                            setTimeout(function () {
                                                ...
                                                $.ajax({
                                                    ...
                                                    success: function (data3) {
                                                        window.location.href = "/Portal/Sheets/Contract/ContractMainMy.aspx?Mode=Work&WorkItemID=" + data3.WorkItemId;
                                                    }
                                                });
                                            }, 1000)
                                        }
                                        // 如果是合同主流程的子流程
                                    } else if (data2.InstanceName != '合同主流程' && data2.ParentInstanceName != null && data2.ParentInstanceName == '合同主流程') {
                                        if (data.IsSaveAction) { }
                                        // 提交的话，则
                                        else {
                                            // 跳转到合同主流程页面
                                            if (data2.ReturnFlg == "Main") {
                                                window.location.href = "/Portal/Sheets/Contract/ContractMainMy.aspx?Mode=Work&WorkItemID=" + data2.ParentWorkItemId;
                                            } else {
                                                $.MvcSheet.ClosePage();
                                            }
                                            
                                        }
                                    } else if (data2.InstanceName != null && data2.InstanceName.indexOf('招标项目主流程') >= 0) {
                                        if (data.IsSaveAction) { }
                                        // 提交的话，则
                                        else {
                                            setTimeout(function () {
                                                ...
                                                $.ajax({
                                                    ...
                                                    async: false,
                                                    success: function (data3) {
                                                        window.location.href = "/Portal/Sheets/bidding/InviteBids.aspx?Mode=Work&WorkItemID=" + data3.WorkItemId;
                                                    }
                                                });
                                            }, 1000)
                                        }
                                    } else if (data2.InstanceName != null && data2.InstanceName.indexOf('协议主流程') >= 0) {
                                        if (data.IsSaveAction) { }
                                        // 提交的话，则
                                        else {
                                            setTimeout(function () {
                                                ...
                                                $.ajax({
                                                    ...
                                                    async: false,
                                                    success: function (data3) {
                                                        window.location.href = "/Portal/Sheets/Agreement/AgreeMent_main/AgreeMent_main.aspx?Mode=Work&WorkItemID=" + data3.WorkItemId;
                                                    }
                                                });
                                            }, 1000)
                                        }
