    ...
    $scope.init = function () {
        ...
        $.ajax({
            ...
            success: function (data, header, config, status) {
                ...
                if ($scope.QueryCode == 'MeansOnlyQueryList') {
                    $scope.GirdColumns.splice(0, 1);
                }
                if ($scope.QueryCode == 'LaborContractOnlyQueryList') {
                    $scope.GirdColumns.splice(0, 1);
                }
    ...
    $scope.BizViewOptions = function () {
        var options = {
                    ...
                    "success": function (json) {
                        ...
                        if ($scope.QueryCode === 'SelectList') {
                            ...
                            for (var i = 0; i < item.length; i++) {
                                ...
                                if (item[i].OperativeMode === '0') {
                                    item[i].OperativeMode = '在职';
                                } else if (item[i].OperativeMode === '2') {
                                    item[i].OperativeMode = '辞职';
                                } else if (item[i].OperativeMode === '3'){
                                    item[i].OperativeMode = '辞退';
                                } else if (item[i].OperativeMode === '4') {
                                    item[i].OperativeMode = '调离';
                                } else if (item[i].OperativeMode === '5') {
                                    item[i].OperativeMode = '退休';
                                } else if (item[i].OperativeMode === '6') {
                                    item[i].OperativeMode = '试用期';
                                }
                            }
                            fnCallback(json);
                        }
            ...
            "fnCreatedRow": function (nRow, aData, iDataIndex) {
                ...
                if ($scope.SchemaCode == 'DataManagement') {
                    var array = $(".dataTables_wrapper.form-inline.dt-bootstrap.no-footer").find('span');
                    $.each(array, function (index, term) {
                        if ($(term).text() == 'ModifiedTime') {
                            $(term).text('更新时间')
                        }
                        if ($(term).text() == 'CreatedBy') {
                            $(term).text('录入人')
                        }
                    });
                ...
                if ($scope.SchemaCode == 'DataManagement' && $scope.sheetCode == undefined) {
                   // var url = '';
                    //alert($scope.QueryCode);
                    if ($scope.QueryCode == 'MeansOnlyQueryList') {
                        //$scope.GirdColumns.splice(0, 1);
                        $(nRow).css('cursor', 'pointer').click(function () {
                            //url = '/Portal/Sheets/Means/MeansMainDetail.aspx?Mode=Work&SchemaCode=' + $scope.SchemaCode + '&BizObjectID=' + aData.ObjectID;
                           // $(".app-aside-right").find('iframe').attr('src', url);
                           // $('.app-aside-right').addClass('show');
                            window.open("/Portal/Sheets/Means/MeansMainDetail.aspx" + "?" + "Mode=Work&SchemaCode=" + $scope.SchemaCode + "&BizObjectID=" + aData.ObjectID);
                        });

                    }
                }
                
                if ($scope.SchemaCode == 'LaborContractManagement' && $scope.sheetCode == undefined) {
                    
                    if ($scope.QueryCode == 'LaborContractOnlyQueryList') {
                        
                        $(nRow).css('cursor', 'pointer').click(function () {
                            window.open("/Portal/Sheets/LaborContract/LaborContractDetail.aspx" + "?" + "Mode=Work&SchemaCode=" + $scope.SchemaCode + "&BizObjectID=" + aData.ObjectID);
                        });
