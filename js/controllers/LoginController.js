app.controller('LoginController', ['$rootScope', '$scope', '$translate',  '$localStorage', '$http', '$location', '$state', '$stateParams', '$timeout', '$interval', 'ControllerConfig',
function ($rootScope, $scope, $translate,$localStorage, $http, $location, $state, $stateParams, $timeout, $interval, Controller) {
    ...
    $rootScope.$on('$viewContentLoaded', function () {
        ...
        if ($scope.user) {
            $scope.userCode = $scope.user.Code;
        }else {
            $scope.userCode = '';
        }
    });
        ...
        .success(function (result, header, config, status) {
            ...
            if (result.Success) {
                ...
                if (redirectUrl && redirectUrl != "" && redirectUrl.indexOf("Redirect.html") != -1) {
                    ...
                    $state.go("app.EditBizObject", {TopAppCode:"YWGL",SchemaCode:"NoticeManager",SheetCode:"NoticeManagerMy",Mode:"Originate",FunctionCode:"Menu_NoticeManager"});
                }
                $scope.LoginSuccess = true;
            }
            else {
