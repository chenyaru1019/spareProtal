            ...
            for (var key in this.ColumnTable) {
                ...
                for (var i = 0; i < this.ColumnTable[key].length - 1; i++) {
                    ...
                    if (key == "ColumnHeaderTableLastLine" || key == "ColumnHeaderTableLastSecondLine" || i == 0 || that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0) {
                        $Th = $('<th  row="' + RowCounter + '" col="' + ColumnCounter + '" data-columncode="' + TempColumnCode + '" rowspan="' + mytd.RowSpan + '" colspan="' + mytd.ColSpan + '" data-value="' + mytd.Value + '" >').html(mytd.Value);
                    }
                    else {
                        $Th = $('<th   row="' + RowCounter + '" col="' + ColumnCounter + '" class="unittd" data-columncode="' + TempColumnCode + '" rowspan="' + mytd.RowSpan + '" colspan="' + mytd.ColSpan + '" data-value="' + mytd.Value + '" >').html(mytd.Value);
        ...
        if (that.Widget.Categories.length == 0 && that.Widget.Columns.length == 0) {
            ...
            for (var key in this.ColumnTable) {
                ...
                for (var i = 0; i < this.ColumnTable[key].length; i++) {
                    var mytd = this.ColumnTable[key][i];
                    if (key == "ColumnHeaderTableLastLine" || key == "ColumnHeaderTableLastSecondLine" || i == 0 || that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0) {
                        $Th = $('<th class="categories"  row="' + RowCounter + '" col="' + ColumnCounter + '" data-columncode="' + TempColumnCode + '" rowspan="' + mytd.RowSpan + '" colspan="' + mytd.ColSpan + '" data-value="' + mytd.Value + '" >').html(mytd.Value);
                    }
                    else {
                        $Th = $('<th class="categories"  row="' + RowCounter + '" col="' + ColumnCounter + '" class="unittd" data-columncode="' + TempColumnCode + '" rowspan="' + mytd.RowSpan + '" colspan="' + mytd.ColSpan + '" data-value="' + mytd.Value + '" >').html(mytd.Value);
                    }
                    $Tr.append($Th.clone());
                    $TableBody.append($Tr);
                }
            ...
            for (var ValueJ = 0; ValueJ < this.ValueTable[0].length; ValueJ++) {
                ...
                if (ValueTableItem == "-" || that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0) {
                    $Td = $('<td  class="columns" rowspan="5" colspan="1" row="' + 0 + '" col="' + ValueJ + '">').html(ValueTableItem).css({  'vertical-align': 'middle' });
                }
                else {
                    $Td = $('<td  class="unittd columns" rowspan="5" colspan="1"  row="' + 0 + '" col="' + ValueJ + '">').html(ValueTableItem).css({ 'vertical-align': 'middle' });
                }
                $tr.append($Td.clone())
            }
            $tdCategory.css({ 'border-right': '2px dashed silver' });
                ...
                for (var RowI = 0; RowI < this.RowTable.length; RowI++) {
                    ...
                    for (var RowJ = 0; RowJ < this.RowTable[RowI].length; RowJ++) {
                        ...
                        if (that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0) {
                            $Th = $('<th class="categories"  data-columncode="' + MyRowTd.ColumnCode + '" data-value="' + MyRowTd.Value + '" rowspan="' + MyRowTd.RowSpan + '" colspan="' + MyRowTd.ColSpan + '">').html(MyRowTd.Value);
                        }
                        else {
                            $Th = $('<th class="unittd categories" data-columncode="' + MyRowTd.ColumnCode + '" data-value="' + MyRowTd.Value + '" rowspan="' + MyRowTd.RowSpan + '" colspan="' + MyRowTd.ColSpan + '">').html(MyRowTd.Value);
                        }
                        $Tr.append($Th.clone());
                    }
                    ...
                    for (var ValueJ = 0; ValueJ < this.ValueTable[RowI].length; ValueJ++) {
                        ...
                        if (ValueTableItem == "-" || that.Widget.LinkageReports == null || that.Widget.LinkageReports.length == 0) {
                            $Td = $('<td class="columns"  rowspan="1" colspan="1" row="' + RowI + '" col="' + ValueJ + '">').html(ValueTableItem);
                        }
                        else {
                            $Td = $('<td  class="unittd columns" rowspan="1" colspan="1"  row="' + RowI + '" col="' + ValueJ + '">').html(ValueTableItem);
